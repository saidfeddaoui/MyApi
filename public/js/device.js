jQuery(document).ready(function() {
    ...
    $('.edit').on('click', function () {
        var id = $(this).data('id');
        $.ajax({
            url: Routing.generate('content_types_edit_device', {id: id}),
            type: "POST",
            ...
            statusCode: {
                ...
                200: function (response) {
                    $("#edit-device-modal .modal-body .form").html(response);
    ...
    $('.delete').on('click', function () {
        ...
        var id = td.data('id');
        swal({
            title: "Voulez-vous vraiement supprimer cet device ?",
            text: "",
            type: "warning",
    ...
    function deleteAction(id, td) {
        $.ajax({
            url: Routing.generate('content_types_delete_device', {id: id}),
            type: "POST",
            error: function (request, status, error) {
                console.log(request.responseText);
            },
