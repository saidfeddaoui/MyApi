jQuery(document).ready(function() {
    ...
    $('.edit').on('click', function () {
        var id = $(this).data('id');
        $.ajax({
            url: Routing.generate('edit_scenario', {id: id}),
            type: "POST",
            ...
            statusCode: {
                ...
                200: function (response) {
                    $("#edit-scenario-modal .modal-body .form").html(response);
                    $("#edit-scenario-modal").modal();
                    return false;
                }
            }
        });
    ...
    $('.delete').on('click', function () {
        ...
        var id = td.data('id');
        swal({
            title: "Voulez-vous vraiement supprimer ce scénario ?",
            text: "",
            type: "warning",
    ...
    function deleteAction(id, td) {
        $.ajax({
            url: Routing.generate('delete_scenario', {id: id}),
            type: "POST",
            error: function (request, status, error) {
                console.log(request.responseText);
            },
